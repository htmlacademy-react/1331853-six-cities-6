...
import {STAR_LIST, ReviewValid} from '../../../const';
...
const UserReview = ({openedOffer, submitCommentOnServer}) => {
  ...
  const formChangeHandler = (evt) => {
    ...
    if (evt.target.name === `rating`) {
      ...
      setUserReview({...userReview, review: commentRef.current.value});
    }
  ...
  return (
    <form className="reviews__form form" action="#" method="post" onChange={(evt) => formChangeHandler(evt)} onSubmit={(evt) => formSubmitHandler(evt)}>
